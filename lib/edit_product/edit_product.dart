...
class _AddProductsWidgetState extends State<EditProductsWidget>
  ...
  Future<bool> getProductElement() async {
    Product editing = await ProductCRUD.getProductToEdit(productID: locator<AuthService>().productSelected);
    print('AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA ${editing}');
    _name = TextEditingController(text: editing.name);
    _description = TextEditingController(text: editing.description);
    _unitPrice = TextEditingController(text: editing.unitPrice);
    _stock = TextEditingController(text: editing.stock);
    storageRef = editing.img;
    newImage = editing.img;
    return true;
  }
                                  ...
                                  children: [
                                    ...
                                    Padding(
                                      ...
                                      child: FFButtonWidget(
                                        onPressed: () async {
                                          await uploadFile();
                                          var result = await ProductCRUD.updateProduct(productID: locator<AuthService>().productSelected, name: _name!.text, description: _description!.text, img: newImage, stock: _stock!.text, unitPrice: _unitPrice!.text);
                                          if(result.code != 200)
                                          {
